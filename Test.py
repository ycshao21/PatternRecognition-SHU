...
def Task01(data: pd.DataFrame) -> None:
    ...
    # classifier = Bayes.Bayes(prior_probs=[0.5, 0.5])
    classifier.Fit(X_train, y_train)

    result = classifier.Predict(X_test)
    for i in range(len(result)):
        print(f"Predicted: {result[i]}, Actual: {y_test[i]}")
...
def Task02(data: pd.DataFrame) -> None:
    ...
    pred_X = np.array([[173, 86], [160, 70], [160, 50], [183, 45]])
    result = classifier.Predict(pred_X)
    print(result)
